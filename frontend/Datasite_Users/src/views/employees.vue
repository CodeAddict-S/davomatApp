<template>
    <table_users :default_filter="'full_name'" @refetch="fetchCourses" :name="'Employee'" :api_link="'users/'" :items="students" :request_success="request_success" :status="status" :default_add_values="add_values" :default_edit_values="edit_values" :values="values" :keys="keys"></table_users>
</template>
    ...
    methods: {
        ...
        fetchStudents() {
            ...
            this.$api.get('users/').then(response => {
                ...
                this.students = result.filter(user=>user.type_user==='employee')
                this.parseStudents()
            }).catch((error)=>{
                console.log(error)
                this.request_success = false
